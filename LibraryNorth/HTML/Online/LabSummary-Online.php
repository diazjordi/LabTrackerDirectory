...
?>
  <?php
require_once("../../../Resources/PHP/MySQL/mysql_connect_labtracker.php");
?>
  <?php
$sqlQuery = "SELECT sub.*   
    FROM (    
...
?>
  <?php
require_once("../../../Resources/PHP/MySQL/mysql_disconnect_labtracker.php");
?>
  ...
  <div align="center" class="container">
    <h3 class="center">
    Information Updated Every 15 minute(s)</h3>
  </div>
...
if ($rowCount > 0) {
    ...
    while ($row = $sqlQueryResults->fetch_row()) {
        ...
        $time = date("g:i A", strtotime($row[6])); // Get time
        echo " <tr> \n";
        echo " <td style=\"background-color:{$color}\" class=\"data-cell\"><font color=\"#000000\">
                      <a href=\"http://10.82.244.203//LabTracker/Maps/{$row[0]}.php\" title=\"{$labName}\">{$labName}</font>
                    </td>\n";
        ...
        echo " <td class=\"time-cell\"><font color=\"#FFCB2F\">
                      <a href=\"http://10.82.244.203/SourceMaps/{$row[0]}.html\" title=\"{$labName}\"><span>{$time}</span></font>
                    </td>\n";
        echo " <td class=\"suppressed-data-cell\"><font color=\"#FFFFFF\">{$row[5]}</font></td>\n";
